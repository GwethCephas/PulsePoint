...
class SearchPagingSourceTest {
    ...
    @Test
    fun `load method returns a Page with data when successful`() = runTest {
        val mockArticles = articles.map { it.toArticle() }
        val newsResponse = NewsResponse("ok", 3, mockArticles)

        coEvery { apiService.getSearchedNews(any(), any(), any(), any()) } returns newsResponse

        val pagingSource = SearchPagingSource(apiService, "query")
